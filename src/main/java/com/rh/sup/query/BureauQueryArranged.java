 ...
 * 司内计划中的事项
 */
public class BureauQueryArranged extends CommonServ {
    ...
    public OutBean query(ParamBean paramBean) {
            ...
            if (bKey && item.getStr("ITEM_CODE").equals(serv.getPKey())) { //主键无论是否列表显示都输出
                ...
                if (item.getInt("ITEM_TYPE") == ServConstant.ITEM_TYPE_TABLE
                    ...
                    if (key.equals("DIRECT") || key.equals("JOINTLY")){
                        ...
                        select.append(",t1.S_WF_USER_STATE AS ").append(item.get("ITEM_CODE"));
        ...
        //增加程序所定义的其他字段
        sql.append(select+" ,S_MTIME, NOT_LIMIT_TIME_REASON ");
        //设定查询表名，并添加查询条件
        sql.append(" FROM SUP_APPRO_BUREAU t1 WHERE (t1.APPLY_STATE = '4' OR " +
                "((SELECT PLAN_STATE FROM SUP_APPRO_PLAN WHERE DEPT_CODE = (SELECT DEPT_CODE \n" +
                "FROM SUP_APPRO_BUREAU_STAFF WHERE BUREAU_ID = t1.ID AND\n" +
                "DEPT_TYPE = '1') AND APPRO_ID = t1.ID) IN ('1','2') AND t1.APPLY_STATE = '4'))");
        //拼接用户所选查询条件
        ...
        //根据特定条件对数据进行分类
        sql.append(" ORDER BY S_MTIME DESC");
    ...
    protected void afterQuery(ParamBean paramBean, OutBean outBean) {
        ...
        for (int i = 0 ; i < beanList.size() ; i++ ){
            ...
            if(bean.getStr("S_UNAME").toString().length()>0){
                String s_uname = "";
                JSONArray array = JSONArray.parseArray(bean.getStr("S_UNAME").toString());
