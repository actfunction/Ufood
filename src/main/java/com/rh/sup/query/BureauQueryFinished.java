 ...
 * 司内已办结事项
 */
public class BureauQueryFinished extends CommonServ {
    ...
    public OutBean query(ParamBean paramBean) {
        ...
        //增加程序所定义的其他字段
        sql.append(select+" ,S_MTIME AS S_MTIME , NOT_LIMIT_TIME_REASON ");
        //设定查询表名，并添加查询条件
        sql.append(" FROM SUP_APPRO_BUREAU t1 WHERE DEALT_TIME IS NOT NULL AND APPLY_STATE = '7' AND S_WF_STATE = '1' ");
        //拼接用户所选查询条件
    ...
    protected void afterQuery(ParamBean paramBean, OutBean outBean) {
        ...
        for (int i = 0 ; i < beanList.size() ; i++ ){
            ...
            if(bean.getStr("DEALT_TIME").length()>0){
                bean.set("DEALT_TIME",bean.getStr("DEALT_TIME").replace("-",""));
            }
        }
