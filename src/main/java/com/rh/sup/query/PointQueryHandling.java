 ...
 * 要点类办理中的事项
 */
public class PointQueryHandling extends CommonServ {
    ...
    public OutBean query(ParamBean paramBean) {
        ...
        for (String key : items.keySet()) {
            ...
            if (bKey && item.getStr("ITEM_CODE").equals(serv.getPKey())) { //主键无论是否列表显示都输出
                ...
                if (item.getInt("ITEM_TYPE") == ServConstant.ITEM_TYPE_TABLE
                    ...
                    if(key.equals("ID")){
                        select.append(",t1.").append(item.get("ITEM_CODE"));
        ...
        //设定查询表名，并添加查询条件
        sql.append(" FROM SUP_APPRO_POINT t1,SY_COMM_TODO t2 WHERE t1.APPLY_STATE = '5' ");
        //拼接用户所选查询条件
