 ...
 * 司内事项登记中
 */
public class BureauQueryRegistration  extends CommonServ {
    ...
    public OutBean query(ParamBean paramBean) {
        ...
        //设定查询表名，并添加查询条件
        sql.append(" FROM SUP_APPRO_BUREAU  WHERE (APPLY_STATE = '1' OR APPLY_STATE = '2' OR APPLY_STATE = '3') ");
        //拼接用户所选查询条件
        ...
        for (int i = 0 ; i < beanList.size() ; i++ ){
            ...
            if (bean.getStr("ID")!=""){
                ...
                if(bean.getStr("SUPERV_ITEM")==""){
                    bean.set("SUPERV_ITEM",bean.getStr("INITIATE_LEADER_NAME")+"领导提出"+bean.getStr("ITEM_SOURCE"));
                }
            }
            //设定当字段为空时显示指定字段
