 ...
 * 司内关注事项
 */
public class BureauQueryConcerns extends CommonServ {
    ...
    public OutBean query(ParamBean paramBean) {
        ...
        for (String key : items.keySet()) {
            ...
            if (bKey && item.getStr("ITEM_CODE").equals(serv.getPKey())) { //主键无论是否列表显示都输出
                ...
                if (item.getInt("ITEM_TYPE") == ServConstant.ITEM_TYPE_TABLE
                    ...
                    }else if( key.equals("S_MTIME") ){
                        select.append(",GAIN_LISTAGG(ID,2) AS ").append(item.get("ITEM_CODE"));
                    }else if( key.equals("S_UNAME") ){
        ...
        //设定查询表名，并添加查询条件
        //拼接查询权限的限制
        sql.append(" FROM SUP_APPRO_BUREAU t1 WHERE  (t1.APPLY_STATE = '5' OR \n" +
        ...
        //拼接查询权限的限制
        sql.append(ProcessingUnitData.getQueryShiro("Bureau"));
        //拼接导出数据所需的特定查询条件
        if (paramBean.get("_searchWhere")!=null){
            sql.append(paramBean.get("_searchWhere"));
        }
        //根据特定条件对数据进行分类
        sql.append(" ORDER BY LIMIT_DATE ASC");
    ...
    protected void afterQuery(ParamBean paramBean, OutBean outBean) {
        ...
        String Appro = "SUP_APPRO_BUREAU_STAFF";
