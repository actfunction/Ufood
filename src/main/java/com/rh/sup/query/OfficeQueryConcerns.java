 ...
 * 署发关注事项
 */
public class OfficeQueryConcerns extends CommonServ {
    ...
    public OutBean query(ParamBean paramBean) {
        ...
        for (String key : items.keySet()) {
            ...
            if (bKey && item.getStr("ITEM_CODE").equals(serv.getPKey())) { //主键无论是否列表显示都输出
                ...
                if (item.getInt("ITEM_TYPE") == ServConstant.ITEM_TYPE_TABLE
                    ...
                    if (key.equals("DIRECT") || key.equals("JOINTLY") || key.equals("BEIGN_END") || key.equals("DETAIL_TEXT")
                            || key.equals("SPEC_FILL") || key.equals("USER_NAME")){
                        select.append(",'' AS ").append(item.get("ITEM_CODE"));
                    }else if(key.equals("GAIN_TEXT")){
                        select.append(",GAIN_LISTAGG(t1.ID,1) AS ").append(item.get("ITEM_CODE"));
                    }else if(key.equals("STATE")){
                        select = ProcessingUnitData.getShowShiro(select,"");
                    }else if(key.equals("S_WF_USER")){
                        select.append(",S_WF_USER_STATE");
                    }else{
        ...
        //增加程序所定义的其他字段
        sql.append(select+",NOT_LIMIT_TIME_REASON ");
        //设定查询表名，并添加查询条件，增加权限设置
        sql.append(" FROM SUP_APPRO_OFFICE t1 WHERE (t1.APPLY_STATE = '5' OR \n" +
                "((SELECT PLAN_STATE FROM SUP_APPRO_PLAN WHERE DEPT_CODE = (SELECT DEPT_CODE \n" +
                "FROM SUP_APPRO_OFFICE_DEPT WHERE OFFICE_ID = t1.ID AND \n" +
                "DEPT_TYPE = '1') AND APPRO_ID = t1.ID AND ROWNUM = 1 ORDER BY S_ATIME DESC) = '3' AND t1.APPLY_STATE = '4')) AND \n" +
                "'SUP_DC_001' NOT IN\n" +
                "\t\t(SELECT ROLE_CODE FROM SY_ORG_ROLE WHERE ROLE_CODE IN (SELECT ROLE_CODE FROM SY_ORG_ROLE_USER\n" +
                "\t\t\t\t\t\t\t\t\t\t\t\t        WHERE USER_CODE IN (SELECT USER_CODE FROM SY_ORG_USER \n" +
                "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tWHERE USER_CODE IN (SELECT OWNER_CODE FROM SY_COMM_TODO WHERE TODO_OBJECT_ID1 = t1.ID)))) \n" +
                "\t\tAND 'SUP_DC_002' NOT IN\n" +
                "\t\t(SELECT ROLE_CODE FROM SY_ORG_ROLE WHERE ROLE_CODE IN (SELECT ROLE_CODE FROM SY_ORG_ROLE_USER\n" +
                "\t\t\t\t\t\t\t\t\t\t\t\t        WHERE USER_CODE IN (SELECT USER_CODE FROM SY_ORG_USER \n" +
                "\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tWHERE USER_CODE IN (SELECT OWNER_CODE FROM SY_COMM_TODO WHERE TODO_OBJECT_ID1 = t1.ID)))) \n" +
                "\t\tAND 'SUP025' NOT IN\n" +
        ...
        //根据特定条件对数据进行分类
        sql.append(" ORDER BY t1.LIMIT_DATE ASC");
    ...
    protected void afterQuery(ParamBean paramBean, OutBean outBean) {
        ...
        for (int i = 0 ; i < beanList.size() ; i++ ){
            ...
            //转换处理当前处理人
            if(bean.getStr("S_WF_USER_STATE").toString().length()>0){
                String s_uname = "";
                JSONArray array = JSONArray.parseArray(bean.getStr("S_WF_USER_STATE").toString());
                for (int j = 0 ; j < array.size() ; j++ ){
                    JSONObject jsonObject = (JSONObject)array.get(j);
                    if (j!= array.size()-1){
                        s_uname += jsonObject.get("N") + "," ;
                    }else{
                        s_uname += jsonObject.get("N") ;
                    }
                }
                bean.set("S_WF_USER",s_uname);
            }
            //调用方法前台渲染数据进行处理
            if (bean.getStr("ID")!=""){
                bean.put("Handling","执行");
                bean = ProcessingUnitData.Processing(bean,Appro);
                /*bean = ProcessingUnitData.Processing_GAIN_TEXT(bean);*/
            }
            //设定当字段为空时显示指定字段
