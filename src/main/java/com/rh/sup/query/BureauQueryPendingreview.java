 ...
 * 司内待审核的事项
 */
public class BureauQueryPendingreview extends CommonServ {
    ...
    public OutBean query(ParamBean paramBean) {
        ...
        for (String key : items.keySet()) {
            ...
            if (bKey && item.getStr("ITEM_CODE").equals(serv.getPKey())) { //主键无论是否列表显示都输出
                ...
                if (item.getInt("ITEM_TYPE") == ServConstant.ITEM_TYPE_TABLE
                    ...
                    if(key.equals("ID")){
                    ...
                    }else if (key.equals("DIRECT") || key.equals("JOINTLY")){
                        ...
                        select = ProcessingUnitData.getShowShiro(select,"office");
                    }else{
        ...
        //设定查询表名，并添加查询条件
        sql.append(" FROM SUP_APPRO_BUREAU t1,SY_COMM_TODO t2 WHERE APPLY_STATE = '6' ");
        //拼接用户所选查询条件
