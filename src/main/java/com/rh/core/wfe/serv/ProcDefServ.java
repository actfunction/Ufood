	...
	protected void beforeQuery(ParamBean paramBean) {
		...
		if (paramBean.getList("_treeWhere").size() > 0) {
			...
			for (int i = 0, len = list.size(); i < len; i++) {
				Bean b = list.get(i);
				if (b.getStr("DICT_ITEM").equals("BELONG_ODEPT")) {
					return;
				}
			}
		}
		...
		String whereSql = paramBean.getQueryExtWhere();
		where.append(whereSql);
		where.append(
				" and (BELONG_ODEPT is null or BELONG_ODEPT = '' or BELONG_ODEPT in (select DEPT_CODE from SY_ORG_DEPT where DEPT_TYPE = 2 and instr(CODE_PATH,'"
						+ odeptCode + "') > 0))");
	...
	private void addNodes(Element rootElement, Bean json, ParamBean wfeBean) {
		...
		for (Bean node : nodes) {
			...
			if (node.isEmpty("nodeDef") || node.getBean("nodeDef").getStr("SERV_ID").length() <= 0) {
				String nodeDef = "{SERV_ID:'" + wfeBean.getStr("SERV_ID") + "',SERV_PID:'" + wfeBean.getStr("SERV_PID")
						+ "',NODE_CODE:'" + node.getStr("key") + "',NODE_NAME:'" + node.getStr("text") + "',NODE_TYPE:'"
						+ (node.getStr("category").equals("start") ? "1" : "2") + "',NODE_IF_AUTOEND:'1'}";
				nodeElement.setText(nodeDef);
			} else {
	...
	private void addLines(Element rootElement, Bean json, ParamBean wfeBean) {
		...
		for (Bean line : lines) {
			...
			if (line.isEmpty("lineDef")) {
				String lineDef = "{LINE_CODE:'" + key + "',LINE_NAME:'" + line.getStr("text") + "',SRC_NODE_CODE:'"
						+ line.getStr("from") + "',TAR_NODE_CODE:'" + line.getStr("to") + "',LINE_IF_RETURN:'1'}";
				lineElement.setText(lineDef);
			} else {
