...
public class Bean extends HashMap<Object, Object> {
     ...
     * @return 对象值
     */
    public final Clob getClob(Object key) {
    	Clob clob = null;
		try {
			clob = new SerialClob(get(key, "").toCharArray());
		} catch (SerialException e) {
			log.error("String转换Clob序列化失败！from：Bean.java");
			e.printStackTrace();
		} catch (SQLException e) {
			log.error("String转换Clob类型失败！from：Bean.java");
			e.printStackTrace();
		}
        return clob;
    }
